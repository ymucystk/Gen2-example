...
## urdfファイルの生成手順
```bash
# gen2用のデータのリポジトリがros-noetic対応なのでubuntu20.04 ros noeticの環境を準備する。
docker pull ros:noetic-ros-base
docker run --net host --ipc host --name gen2_robot_urdf -it ros:noetic-ros-base

sudo apt update
sudo apt install -y git ros-noetic-catkin python3-catkin-tools
...
cd src
# gen2用のデータのリポジトリをクローン
git clone -b noetic-devel https://github.com/Kinovarobotics/kinova-ros.git kinova-ros
cd ..
...
rosrun xacro xacro ~/catkin_ws/src/kinova-ros/kinova_description/urdf/j2n6s300_standalone.xacro > gen2_robot.urdf
```
## robot-loader用ファイルの生成手順
```bash
# urdfを生成したubuntu20.04では作業継続できないので新規にubuntu24.04のコンテナを起こす。
docker pull ros:jazzy-ros-base
docker run --net host --ipc host --name gen2_robot_lv2 -it ros:jazzy-ros-base

# ホスト側でurdfを作ったdockerコンテナからこのコンテナに作業フォルダごとコピーする。
docker cp gen2_robot_urdf:/root/catkin_ws ./
docker cp ./catkin_ws gen2_robot_lv2:/root/catkin_ws

# コンテナに戻る
cd ~
sudo apt update
sudo apt install -y ros-dev-tools ros-jazzy-desktop-full wget
...
# rosで生成したurdfファイルをコピー
cp ~/catkin_ws/gen2_robot.urdf ./
...
# URDF ツリーを個別のチェーンに分割
./a/splitUrdfTree.sh gen2_robot.urdf
...
./a/cut-joint-map.sh urdfmap.json --from j2n6s300_link_base --to j2n6s300_end_effector
...
DSRDir=~/catkin_ws/src/kinova-ros/kinova_description

# メッシュファイルへのパスの作成
for path in "${Meshes[@]}"; do path=`echo $path | sed 's|^package://kinova_description/||'`; ln -s $DSRDir/$path .; done
# gltfファイルの生成
...
# robot-loader用データ
mkdir -p ./public/gen2
cp urdfmap_cut.json ./public/gen2/urdf.json
cp linkmap.json ./public/gen2/linkmap.json
cp update.json ./public/gen2/update.json
cp -r meshes/out/*.* ./public/gen2/
...
# shapeList.json の編集
echo -e '[\n  [ "table.ply", "base.bbox.ply" ],\n  [ "shoulder.bbox.ply", "ring_big.bbox.ply" ],\n  [ "arm.bbox.ply", "ring_big.bbox.ply" ],\n  [ "forearm.bbox.ply", "ring_big.bbox.ply" ],\n  [ "wrist.bbox.ply", "ring_small.bbox.ply" ],\n  [ "wrist.bbox.ply", "ring_small.bbox.ply" ],\n  [ "hand_3finger.bbox.ply", "ring_small.bbox.ply" ],\n  [  ]\n]' > shapeList.json
...
cp ./shapes.json ../public/gen2/
cp ../testPairs.json ../public/gen2/
...
# robot-loader用データ
cp -r meshes/out/*.bbox.* ./public/gen2/

```

## 以下、旧版の手順書

urdfファイルの生成手順には、ubuntu 20.04 LTS および ROS Noetic が必要です。<br>
```bash
docker pull ros:noetic-ros-base
docker run --net host --ipc host --name gen2_robot_urdf -it ros:noetic-ros-base
...
0. ros ワークスペースがない場合は作成します。**上書きは不可です。**
   ```bash
   mkdir -p ~/catkin_ws/src
   cd ~/catkin_ws
   catkin init
...
1. Kinova git リポジトリを ros ワークスペースの `src` フォルダにクローンします。
   ```bash
   cd ~/catkin_ws/src
   git clone -b noetic-devel https://github.com/Kinovarobotics/kinova-ros.git kinova-ros
   cd ..
...
4. xacro を使用して ROS URDF ファイルを作成します。
   ...
   rosrun xacro xacro ~/catkin_ws/src/kinova-ros/kinova_description/urdf/j2n6s300_standalone.xacro > gen2_robot.urdf
   ```
   以下の手順には、ubuntu24.04、ROS2、blender3.6、meshlab/meshlabserver、および Node.js が必要です。<br>
...
4. この作業ディレクトリに戻り、まだの場合はスクリプトの[リポジトリ](https://github.com/ymucystk/Gen3-Lite-example)をクローンします。<br>
   **※ 前ステップで生成した catkin_ws フォルダを～配下にコピーします。**<br>
   **※ 前ステップで生成した ROS URDF ファイルをこの作業ディレクトリにコピーします。**<br>
   ```bash
   mv ./gen2_robot.urdf <this_working_directory>
   ```
   ```bash
...
12. メッシュファイルを `meshes` フォルダにシンボリックリンクまたはコピーします。**上書き不可です。**<br>
   クローン直後の状態で既に`meshes` フォルダがある場合は`meshes_org` 等にリネームして退避します。<br>
    ...
    for path in "${Meshes[@]}"; do
      path=`echo $path | sed 's|^package://kinova_description/||'`
      ln -s $DSRDir/$path .
    done
    ```
...
15. 最後に、`urdfmap_cut.json` を `urdf.json` に名前変更し、`urdf.json`、`linkmap.json`、`update.json`、および `meshes/out/` フォルダ内のファイルを `public/gen2/` フォルダまたは他の任意の希望のフォルダに移動します。<br>
    ```bash
      mkdir -p ./public/gen2
      cp urdfmap_cut.json ./public/gen2/urdf.json
      cp linkmap.json ./public/gen2/linkmap.json
...
この作業はubuntu20.03ではできないのでここまでで生成したmeshes内のデータをubuntu24.03にコピーしてから作業します。

以下の手順には、meshlab、が必要です。meshlab は apt を使用してインストールできます。:
```bash
...
7. `shapeList.json` から `shapes.json` ファイルを作成します。
   ...
   mv output.json shapes.json
   ```
   <!--これで、gen2 ロボットのコライダーを定義する `shapes.json` ファイルができました。<br>
...
9. 最後に、`shapes.json` と `testPairs.json` を `public/gen2/` フォルダまたは他の任意の希望のフォルダに移動します。<br>
    ```bash
    cp ./shapes.json ../public/gen2/
    cp ../testPairs.json ../public/gen2/
    ```
...
10. コライダーの STL ファイルからコライダー用の glTF ファイルを作成します。 (`cd meshes/`)
    ...
    cd ../..
    ```
	`meshes/out/` フォルダで生成された glTF ファイルと bin ファイルを `public/gen2/` フォルダまたは他の任意の希望のフォルダに移動します。
    ```bash
    cp -r meshes/out/*.bbox.* ./public/gen2/
    ```
...
コピーした`App.tsx`ファイル内の`ur5e`の記述を`gen2`に変更します。(3か所)<br>
また、`ik-worker`の配列を８個に変更します。<br>
```
ik-worker={`0, ${Math.PI}, ${deg90}, 0, ${deg90}, 0, 0.1, 0.1`}
```
